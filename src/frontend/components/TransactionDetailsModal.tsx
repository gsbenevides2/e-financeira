...
export const TransactionDetailsModal: React.FC<TransactionDetailsModalProps> = (
  ...
  const loadData = async () => {
    try {
      ...
      const relatedPromise = apiClient.api.transactions({
        id: transaction.id,
      }).related.get()
      const accountsPromise = apiClient.api.accounts.get()
      const categoriesPromise = apiClient.api.categories.get()

      const [relatedResponse, accountsResponse, categoriesResponse] = await Promise.all([
        relatedPromise,
        accountsPromise,
        categoriesPromise,
      ])
      const checkRelated = isSuccessStatus(relatedResponse.status) && relatedResponse.data
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto border border-gray-700">
        ...
        <div className="p-6">
          ...
          {/* Additional Details */}
          {(transaction.description || transaction.address
