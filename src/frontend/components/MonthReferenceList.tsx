...
export function MonthReferenceList() {
	...
	const loadData = useCallback(async () => {
		try {
			...
			const response = await apiClient.api["month-references"].get();
	...
	const handleDelete = async (id: string) => {
		if (!confirm("Tem certeza que deseja excluir este mês de referência?")) {
			return;
		}
	...
	return (
		<div className="space-y-4">
			{showAddForm && (
				<MonthReferenceForm onClose={handleCloseForm} onSave={handleSave} />
			)}
				...
				<table className="min-w-full divide-y divide-gray-700">
					...
					<tbody className="bg-gray-800 divide-y divide-gray-700">
						{monthReferences.map((monthRef) => (
							<tr
								key={monthRef.id}
								className={monthRef.active ? "" : "opacity-60"}
							>
								...
								<td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
									...
									<Button
										variant="ghost"
										size="sm"
										onClick={() => handleDelete(monthRef.id)}
