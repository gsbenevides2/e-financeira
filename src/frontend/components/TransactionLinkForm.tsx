...
export function TransactionLinkForm({
	transaction,
	onClose,
}: TransactionLinkFormProps) {
	...
	const loadData = useCallback(async () => {
		try {
			...
			const linkedPromise = apiClient.api
				.transactions({
					id: transaction.id,
				})
				.related.get();
			...
			const checkLinked =
				isSuccessStatus(linkedResponse.status) && linkedResponse.data;
			...
			const available = transactionsData.filter(
				(t: Transaction) => t.id !== transaction.id && !linkedIds.has(t.id),
			);
	...
	const filteredTransactions = availableTransactions.filter(
		(t) =>
			searchQuery === "" ||
					...
					<div>
						...
						{linkedTransactions.length === 0 ? (
							<div className="text-center py-8 bg-gray-800 rounded-lg border border-gray-700">
								<div className="text-gray-400">Nenhuma transação vinculada</div>
								<p className="text-sm text-gray-500 mt-2">
									Vincule transações relacionadas usando a lista abaixo
								</p>
							</div>
						) : (
									...
									>
										<div>
											<div className="flex items-center space-x-2">
												...
												<span
													className={`text-sm ${
														t.value >= 0 ? "text-green-400" : "text-red-400"
													}`}
												>
