...
interface MonthReferenceFormProps {
    onClose: () => void;
    onSave: () => void;
}

export const MonthReferenceForm: React.FC<MonthReferenceFormProps> = ({
    onClose,
    onSave,
}) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await apiClient.api["month-references"].post({
                month: formData.month,
                year: formData.year,
            });

            if (!isSuccessStatus(response.status)) {
                throw new Error("Erro ao criar mês de referência");
            }
            
            onSave();
        } catch (err) {
            setError("Erro ao criar mês de referência");
            console.error(err);
        } finally {
            setLoading(false);
        }
