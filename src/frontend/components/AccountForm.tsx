...
export function AccountForm({ onClose }: AccountFormProps) {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			setLoading(true);
			setError(null);
			const response = await apiClient.api.accounts.post({
				name: name.trim(),
				accountType,
			});
				...
				<form onSubmit={handleSubmit} className="p-6 space-y-4">
					...
					<div popover="auto">
						...
						<p className="text-xs text-gray-400 mt-1">
							Escolha um nome descritivo para facilitar a identificação da
							conta.
						</p>
					</div>
					...
					<div>
						...
						<select
							...
							disabled={loading}
						>
							<option value={AccountTypes.DEBIT}>Conta Débito</option>
							<option value={AccountTypes.CREDIT}>Cartão de Crédito</option>
						</select>
						<p className="text-xs text-gray-400 mt-1">
							Selecione o tipo de conta para melhor organização das suas
							finanças.
						</p>
					</div>
