...
export class MonthReferenceService {
  ...
  static async update(dto: UpdateMonthReferenceDto): Promise<MonthReference | undefined> {
    ...
    if (dto.month !== undefined)
      updateData.month = dto.month
    if (dto.year !== undefined)
      updateData.year = dto.year
    if (dto.active !== undefined)
      updateData.active = dto.active

    const [monthReference] = await db.update(monthReferences).set(updateData).where(eq(monthReferences.id, dto.id)).returning()
  ...
  static async getActiveOnly(): Promise<MonthReference[]> {
    const monthReferencesList = await db.select().from(monthReferences).where(eq(monthReferences.active, true)).orderBy(monthReferences.year, monthReferences.month)
