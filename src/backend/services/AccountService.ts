...
export class AccountService {
	...
	static async delete(id: UUID): Promise<void> {
		...
		if (transactionCount.length > 0) {
			throw new Error(
				"Não é possível excluir uma conta que possui transações. Mova as transações para outra conta primeiro.",
			);
		}
	...
	static async listTransactions(
		...
		const whereConditions = [eq(transactions.accountId, accountId)];
		...
		if (minValue !== undefined) {
			filteredTransactions = filteredTransactions.filter(
				(tx) => tx.value >= minValue,
			);
		}

		if (maxValue !== undefined) {
			filteredTransactions = filteredTransactions.filter(
				(tx) => tx.value <= maxValue,
			);
		}
	...
	static async calculateBalance(accountId: UUID): Promise<number> {
		...
		const total = Number.parseFloat(result[0]?.total || "0");
	...
	static async removeTransaction(
		...
		throw new Error(
			"Para remover uma transação de uma conta, mova-a para outra conta ou exclua-a",
		);
	}
	...
	static async getAccountsByType(type: AccountTypes): Promise<Account[]> {
		const accountsList = await db
			.select()
			.from(accounts)
			.where(eq(accounts.accountType, type))
			.orderBy(accounts.name);
	...
	static async getDebitAccounts(): Promise<Account[]> {
		return AccountService.getAccountsByType(AccountTypes.DEBIT);
	}

	static async getCreditAccounts(): Promise<Account[]> {
		return AccountService.getAccountsByType(AccountTypes.CREDIT);
	}
	...
	static async getAccountSummary(
		...
		let monthlyTotal = 0;
		if (month && year) {
			monthlyTotal = await AccountService.getTotalForMonth(
				accountId,
				month,
				year,
			);
	...
	static async getAllAccountsSummary(
		month?: number,
		year?: number,
	): Promise<AccountSummary[]> {
		const accountsList = await AccountService.getAll();
	...
	static async getAccountsWithBalance(): Promise<
